...
pip install --no-cache-dir -r "${SCRIPT_DIR}/requirements/requirements.txt" --use-deprecated=legacy-resolver --trusted-host=pypi.python.org --trusted-host=pypi.org --trusted-host=files.pythonhosted.org
...
if [[ "x${EXTRA_REQUIREMENTS_TXT}" != "x" ]]; then
	pip install --no-cache-dir -r "${EXTRA_REQUIREMENTS_TXT}" --use-deprecated=legacy-resolver --trusted-host=pypi.python.org --trusted-host=pypi.org --trusted-host=files.pythonhosted.org
...
echo '::group::🐶 🕵️ Checking file format'
...
echo "name=sqlfluff-exit-code::1" >> $GITHUB_STEP_SUMMARY
echo "name=reviewdog-return-code::1" >>$GITHUB_OUTPUT
echo "name=reviewdog-return-code::1" >>$GITHUB_STEP_SUMMARY
exit 1
fi
done
echo '::endgroup::'
